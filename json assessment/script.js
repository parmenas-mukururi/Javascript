...
array.forEach(function (event) {
  const mySection = document.querySelector("body");
  const myMain = document.createElement("main");
  const myImage = document.createElement("img");
  const myTitle = document.createElement("h3");
  const myDate = document.createElement("p");
  const myCompany = document.createElement("p");
  const myPrice = document.createElement("p");
  const myBtn = document.createElement("button");

  myImage.src = event.imageUrl;
  myTitle.textContent = event.title;
  myDate.textContent = event.date;
  myCompany.textContent = event.company;
  myPrice.textContent = `Ksh ${event.price}`;
  myBtn.textContent = `Buy Now`;

  myMain.appendChild(myImage);
  myMain.appendChild(myTitle);
  myMain.appendChild(myDate);
  myMain.appendChild(myCompany);
  myMain.appendChild(myPrice);
  myMain.appendChild(myBtn);
  mySection.appendChild(myMain);
});
